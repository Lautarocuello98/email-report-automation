...
from email_report_automation.validation import normalize_sales, validate_row


def test_normalize_sales_accepts_numbers_and_comma_strings() -> None:
    assert normalize_sales(1200) == 1200.0
    assert normalize_sales("1,200.50") == 1200.5
...
def test_validate_row_returns_clean_data() -> None:
    ...
    name, email, sales = validate_row(row=row, csv_line=2)
